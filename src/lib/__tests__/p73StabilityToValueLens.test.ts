  ...
  function walk(dir: string, out: string[] = []): string[] {
    for (const name of readdirSync(dir, { withFileTypes: true })) {
      ...
      if (name.isDirectory()) {
        ...
        if (full.endsWith("p76ToolSpecificReportFramework.test.ts")) continue;
       if (full.endsWith("p78GuidedLandingWalkthroughRegistry.test.ts")) continue;
      ...
      if (full.endsWith("p82FinalLaunchSmokeTest.test.ts")) continue;
      if (full.endsWith("p84NewAccountsApprovalQueue.test.ts")) continue;
        out.push(full);
      }
    }
    return out;
