...
- Industry Brain Deep Expansion + Variable Coverage Hardening — deepened seeded Industry Brain entries to ~10 per industry across revenue, demand, conversion, operational, financial visibility, owner-dependence, staffing, customer handoff, software/evidence, margin/profitability, repair-map, and tool-readiness categories. Added uniqueness index on `(industry_key, title)` to prevent duplicate seeding. Cannabis / MMJ / MMC kept strictly to dispensary / retail / operations logic — no healthcare, patient-care, insurance, claims, HIPAA, or clinical workflow framing. Softened "100% default tool coverage" / "100% default coverage" copy on `ToolCatalog.tsx` and `ToolMatrix.tsx` to "Default tools mapped" / "X% of default tools mapped". Admin route protection, RLS, and admin-only defaults unchanged. See docs/rgs-industry-brain-deep-expansion.md.
