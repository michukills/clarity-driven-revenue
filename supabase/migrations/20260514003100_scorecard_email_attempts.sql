-- P93E-E4B — Scorecard email attempt history + admin resend RPC.
--
-- Adds an append-only attempts table so admin-triggered resends preserve
-- a full audit trail (actor, timestamp, status, safe failure reason,
-- provider message id when safe). Automatic dispatcher continues to
-- update scorecard_runs.follow_up_email_status via the existing
-- admin_record_scorecard_email_result RPC; this table is additive.
...
-- Admin-only read; nothing else may read this table directly.
...
-- Writes only via service-role-backed RPCs below; no client INSERT/UPDATE/DELETE policies.

-- Service-role RPC: append an attempt row. Called from edge functions only.
