-- Launch-blocker repair: portal intake approval + new-signup linking.
--
-- Root issue guarded here:
--   customers.lifecycle_state defaults to 'lead', and the live lead-stage
--   industry guard rejects lead rows with no industry unless
--   needs_industry_review=true. Portal requests can legitimately arrive with
--   "industry not provided", so every approval/create path must explicitly
--   mark the row for industry review instead of dead-ending.
...
BEGIN
  ...
  v_industry := COALESCE(NULLIF(btrim(_override_industry),''), v_req.industry);
  ...
  IF v_new_status IN ('approved_client','approved_demo') THEN
    ...
    IF v_existing_customer_id IS NULL THEN
      INSERT INTO public.customers (
        ...
        v_industry,
        CASE WHEN v_needs_industry_review
    ...
    ELSE
      UPDATE public.customers
         SET user_id = v_req.user_id,
             ...
             status = 'active',
             needs_industry_review = COALESCE(v_industry, industry) IS NULL,
             industry_confirmed_by_admin = false,
             industry_intake_source = 'portal_access_request',
             industry_intake_value = COALESCE(v_industry, industry),
             industry_review_notes = CASE WHEN COALESCE(v_industry, industry) IS NULL
               THEN 'Portal intake did not include industry. Review before confirming industry, payment, portal access, or delivery scope.'
               ELSE 'Industry came from portal intake. Confirm before enabling industry-specific tools.'
             END,
...
BEGIN
  ...
  v_industry := NULLIF(btrim(v_req.industry), '');
